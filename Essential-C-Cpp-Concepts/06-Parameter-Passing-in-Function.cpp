...
void swap_pass_by_reference(int &a, int &b) {
   ...
   * function, when this function is called its just pasted inside the "main"
   * function. So it's like monolithic program. So we can say that though source
   * code is modular the inside working of the function is monolithic.*/
  int temp;
  temp = a;
  a = b;
  b = temp;
